...
app.get("/api/master/stats", authenticate, requirePermission("admin"), async (req, res) => {
  try {
    const usersDb = await readKey('users', { users: [] });
    const crmUsers = Array.isArray(usersDb.users) ? usersDb.users : [];
    const diyDb = await readKey('diy_users', { users: [] });
    const diyUsers = Array.isArray(diyDb.users) ? diyDb.users : [];
    const consumersDb = await readKey('consumers', { consumers: [] });
    const consumers = Array.isArray(consumersDb.consumers) ? consumersDb.consumers : [];
    ...
    for (const u of crmUsers) {
      ...
      if (!tenantMap[tid]) tenantMap[tid] = { count: 0, roles: {} };
      tenantMap[tid].count++;
      const role = u.role || 'member';
      tenantMap[tid].roles[role] = (tenantMap[tid].roles[role] || 0) + 1;
    }

    const crmByRole = {};
    for (const u of crmUsers) {
      const role = u.role || 'member';
      crmByRole[role] = (crmByRole[role] || 0) + 1;
    }

    const diyByPlan = {};
    for (const u of diyUsers) {
      const plan = u.plan || 'free';
      diyByPlan[plan] = (diyByPlan[plan] || 0) + 1;
    }

    const consumersByStatus = {};
    for (const c of consumers) {
      const status = c.status || 'active';
      consumersByStatus[status] = (consumersByStatus[status] || 0) + 1;
    }

    const crmSignups30d = crmUsers.filter(u => {
      const d = u.createdAt ? new Date(u.createdAt).getTime() : 0;
      return d >= thirtyDaysAgo;
    }).length;

    const diySignups30d = diyUsers.filter(u => {
      const d = u.createdAt ? new Date(u.createdAt).getTime() : 0;
      return d >= thirtyDaysAgo;
    }).length;
    ...
    res.json({
      ...
      totalDiyUsers: diyUsers.length,
      totalConsumers: consumers.length,
      activeTenants: Object.keys(tenantMap).length,
      crmByRole,
      diyByPlan,
      consumersByStatus,
      crmSignups30d,
...
app.get("/api/master/revenue", authenticate, requirePermission("admin"), async (req, res) => {
  try {
    const invoicesDb = await readKey('invoices', { invoices: [] });
    const invoices = Array.isArray(invoicesDb.invoices) ? invoicesDb.invoices : [];
    ...
    if (pgPool) {
      try {
        ...
        for (const row of subsResult.rows) {
          const unitAmount = (row.unit_amount || 0) / 100;
          const prodMeta = typeof row.product_metadata === 'string'
            ? JSON.parse(row.product_metadata) : (row.product_metadata || {});
          const subMeta = typeof row.metadata === 'string'
            ? JSON.parse(row.metadata) : (row.metadata || {});
          const mode = subMeta.mode || prodMeta.mode || 'crm';

          if (mode === 'diy') {
            diyRevenue += unitAmount;
            stripeSubscriptions.diy.push({
              id: row.id, status: row.status, product: row.product_name, amount: unitAmount
            });
          } else {
            crmRevenue += unitAmount;
            stripeSubscriptions.crm.push({
              id: row.id, status: row.status, product: row.product_name, amount: unitAmount
            });
          }
        }
      } catch (err) {
        console.warn('Master revenue Stripe query fallback:', err.message);
      }
    }

    const totalMrr = crmRevenue + diyRevenue;
    ...
    res.json({
      ...
      collectionRate,
      invoicesLast30d: Math.round(invoicesLast30d * 100) / 100,
      totalMrr: Math.round(totalMrr * 100) / 100,
      crmMrr: Math.round(crmRevenue * 100) / 100,
      diyMrr: Math.round(diyRevenue * 100) / 100,
      crmSubscriptionCount: stripeSubscriptions.crm.length,
      diySubscriptionCount: stripeSubscriptions.diy.length,
      profitLoss: Math.round((totalCollected - outstanding) * 100) / 100
    });
  } catch (err) {
    console.error('Master revenue error:', err);
    res.status(500).json({ ok: false, error: 'Failed to load revenue data' });
  }
});

app.get("/api/master/news", authenticate, async (_req, res) => {
  try {
    const db = await readKey('platform_news', { announcements: [] });
    const announcements = Array.isArray(db.announcements) ? db.announcements : [];
    announcements.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    res.json({ ok: true, announcements });
  } catch (err) {
    console.error('Master news load error:', err);
    res.status(500).json({ ok: false, error: 'Failed to load announcements' });
  }
});

app.post("/api/master/news", authenticate, requirePermission("admin"), async (req, res) => {
  try {
    ...
    await writeKey('platform_news', db);
    res.json({ ok: true, announcement });
  } catch (err) {
    console.error('Master news create error:', err);
    res.status(500).json({ ok: false, error: 'Failed to create announcement' });
  }
});

app.delete("/api/master/news/:id", authenticate, requirePermission("admin"), async (req, res) => {
  try {
    const db = await readKey('platform_news', { announcements: [] });
    if (!Array.isArray(db.announcements)) db.announcements = [];
    ...
    console.error('Master news delete error:', err);
...
app.get("/api/diy/news", async (_req, res) => {
  try {
    const db = await readKey('platform_news', { announcements: [] });
    const announcements = Array.isArray(db.announcements) ? db.announcements : [];
    announcements.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
