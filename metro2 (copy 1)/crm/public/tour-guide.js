...
const PAGE_TOURS = {
  dashboard: {
    ...
    steps: [
      ...
      { target: '[data-tour="news"], #newsFeed', title: 'Live Activity Feed: Your Business Pulse', body: "This real-time feed shows everything happening across your operation as it happens. New client added, letters generated, dispute responses received, payments processed, system updates — all in chronological order. If something important happened while you were away, you'll see it here first. Think of it as your business news ticker — a live log of every significant event." },
      { target: '[data-tour="messages"], #msgList', title: 'Notifications Center: Every Alert, One Place', body: "All notifications and client messages flow into this single feed. Here's what each notification type means so you always know what action to take:<br><br><strong>🔔 New Lead</strong> — Someone filled out your intake form or was added manually. Follow up within 5 minutes for best conversion.<br><strong>👤 New Client</strong> — A lead converted to a paying client. Time to start onboarding and upload their credit report.<br><strong>✉️ Letter Generated</strong> — A dispute letter was created and is ready to review, download, and mail.<br><strong>📬 Letter Sent</strong> — A letter was marked as mailed. Bureau's 30-day investigation window begins now.<br><strong>📩 Bureau Response</strong> — A bureau replied to a dispute. Open the tracker to review and decide next steps.<br><strong>📂 Document Uploaded</strong> — A client uploaded a file through their portal (credit report, ID, bank statement).<br><strong>⏰ Task Reminder</strong> — A scheduled follow-up or deadline is due. Review before it falls through the cracks.<br><strong>⚠️ System Alert</strong> — An important platform notification (API issue, billing alert, or feature update)." },
  ...
  library: {
    ...
    steps: [
      { target: '.library-hero, [data-tour="library-hero"]', title: 'Letter Library: Your Legal Dispute Arsenal', body: "Every dispute letter you'll ever need is organized here by category, violation type, and bureau. These aren't generic templates — each is written around specific legal violations under three key laws: the FCRA (Fair Credit Reporting Act — the federal law that gives consumers the right to dispute inaccurate credit information), the FDCPA (Fair Debt Collection Practices Act — the law that governs how debt collectors must behave), and Metro-2 compliance standards (the technical data format all three bureaus use to report account data — violations of this format give grounds for deletion)." },
      { target: '#templatePanel, [data-tour="templates"]', title: 'Template Categories: The Right Letter for Every Situation', body: "Templates are organized by dispute type so you find the right one fast. Initial Disputes (Round 1) are sent first to bureaus after uploading a credit report. Validation Letters go to debt collectors asking them to prove the debt is valid. Goodwill Letters politely ask for removal based on an otherwise good payment history. Legal Threat Letters escalate with formal legal citations when earlier rounds fail. Bureau-Specific Letters address each bureau (TransUnion, Experian, Equifax) individually when needed." },
      { target: '#sequencePanel, [data-tour="sequences"]', title: 'Automated Letter Sequences: Set It and Forget It', body: "Letter Sequences are pre-scheduled multi-round dispute campaigns. Round 1 goes out immediately. Round 2 fires automatically 35 days later — this timing is precise because the FCRA gives bureaus 30 days to investigate, so Round 2 lands just after that window. Round 3 escalates with stronger legal language if the item still isn't removed. Configure it once per client and the system sends each round at exactly the right time — no calendar reminders needed." },
      { target: '#editorPanel, [data-tour="editor"]', title: 'Smart Letter Editor: Professional Results Every Time', body: "When you select a template, the editor opens with your client's details already filled in via merge fields — their name, address, account numbers, and the specific violation language. Merge fields pull data from the client profile automatically, so you're never copy-pasting. The editor also flags missing required disclosures (required under CROA — the Credit Repair Organizations Act — which mandates specific language in all dispute correspondence) before you generate, ensuring every letter is legally complete." }
    ]
  },
  billing: {
    label: 'Billing',
  ...
  letters: {
    ...
    steps: [
      { target: '[data-tour="letter-list"], .letter-list, .letter-queue', title: 'Dispute Letter Queue: Your Complete Dispatch Log', body: "Every dispute letter you've ever generated is cataloged here — like a permanent mailing log for your business. Each entry shows the client name, letter type, which bureau or creditor it targets (TransUnion, Experian, Equifax, or a specific creditor like Capital One), the date it was created, and its current status: Draft, Ready to Send, Sent, or Response Received. Click any entry to preview, download, or start a new round." },
      { target: '[data-tour="letter-actions"], .letter-actions, .action-buttons', title: 'Letter Actions: Send, Track, and Escalate', body: "For each letter, you have several key actions. Download as PDF to print and mail via certified mail — certified mail (sent through USPS with a tracking number and signature confirmation) creates a legal paper trail proving the bureau received your dispute, which is critical if they claim no response is needed. Mark as Sent once mailed. When the bureau responds (within the 30-day FCRA window — the Fair Credit Reporting Act requires bureaus to investigate within 30 days), mark it as 'Response Received.' If the dispute wasn't resolved, generate the next round — all timestamped for a complete audit record." },
      { target: '[data-tour="letter-generate"], .generate-btn', title: 'Generate New Dispute Letters', body: "Click here to start the letter generation workflow: select a client, choose which negative items to dispute (from their uploaded credit report), pick a template from the library or let AI write a custom one, preview and make any edits, then generate. Create one letter at a time for precision work — or batch-generate across multiple clients and accounts for maximum efficiency." }
    ]
  },
  tradelines: {
    label: 'Tradelines',
    ...
    steps: [
      { target: '[data-tour="tradeline-upload"], .upload-section, .report-upload', title: 'Credit Report Upload: Analysis Begins Here', body: "A tradeline is any account that appears on a credit report — credit cards, auto loans, mortgages, medical collections, or student loans. Upload your client's credit report (PDF or HTML format) and our Metro-2 engine analyzes every tradeline automatically. Metro-2 is the technical data standard that all three credit bureaus (TransUnion, Experian, Equifax) use to report account data — and it has strict formatting rules that, when violated, give your client legal grounds to dispute and request deletion. Processing takes seconds and covers all three bureaus at once." },
      { target: '[data-tour="tradeline-results"], .tradeline-list, .violations-panel', title: 'Violation Analysis Results: Your Legal Ammunition', body: "After processing, violations are organized by severity. Critical violations include wrong Social Security numbers or accounts that don't belong to the client — these often result in immediate deletion under the FCRA (Fair Credit Reporting Act). Major violations include incorrect balances, wrong payment history dates, or re-aged debt (reporting a debt as newer than it really is, which resets the 7-year reporting clock illegally). Minor violations include Metro-2 formatting errors. Each violation is documented with the specific law citation you reference in your dispute letter." },
      { target: '[data-tour="tradeline-cross-bureau"], .cross-bureau', title: 'Cross-Bureau Comparison: Catching Inconsistencies', body: "The three credit bureaus (TransUnion, Experian, Equifax) are supposed to report the same account the same way. When they don't — that's an automatic violation under the FCRA. This panel compares each account simultaneously across all three bureaus. If Equifax shows a $4,500 balance on an account but Experian shows $3,200 for the same account, at least one is provably wrong. Cross-bureau inconsistencies are often your fastest path to deletions because they're the hardest for bureaus to defend." },
      { target: '[data-tour="tradeline-generate"], .generate-letters-btn', title: 'Generate Letters from Violations', body: "After reviewing violations, click here to auto-generate the right dispute letters for each finding. The system automatically matches the most effective template to each violation type, addresses it to the correct bureau, and pre-fills specific account details and legal citations (such as 'Pursuant to 15 U.S.C. § 1681i of the Fair Credit Reporting Act...'). One click turns a complete report analysis into a full set of ready-to-send dispute letters." }
    ]
  },
  schedule: {
    label: 'Schedule',
  ...
  marketing: {
    ...
    steps: [
      ...
      { target: '[data-tour="marketing-templates"], .template-section', title: 'Marketing Templates: Legally Compliant Frameworks', body: "These pre-written templates for social media posts, email sequences, landing pages, and ad copy are all written to comply with FTC (Federal Trade Commission — the federal agency that regulates advertising) rules for credit repair marketing. The FTC prohibits guaranteeing specific score increases or promising outcomes. Every template here avoids those pitfalls while still being persuasive. Customize with your branding, testimonials (carefully — you must not fabricate these), and unique value proposition." },
  ...
  'my-company': {
    ...
    steps: [
      { target: '[data-tour="company-info"], .company-info', title: 'Business Identity: Your Professional Foundation', body: "Your company name, address, phone number, and email are printed on every dispute letter you send. This matters legally and practically: bureaus use your return address to respond to disputes, and professional businesses get taken more seriously than disputes coming from a personal Gmail address. Make sure this info matches your business registration exactly." },
      { target: '[data-tour="company-logo"], .logo-section', title: 'Logo & Branding: Look Premium Instantly', body: "Upload your company logo and it automatically appears on every letter you generate, your client portal login page, your email templates, and your marketing materials. This creates visual consistency — the hallmark of a professional operation. Studies consistently show that clients are 3x more likely to sign a contract when they receive a polished-looking proposal vs. a plain text document with no branding." },
      { target: '[data-tour="company-compliance"], .compliance-section', title: 'Compliance & Legal: Stay Protected', body: "Credit repair companies must comply with CROA — the Credit Repair Organizations Act — a federal law that governs exactly what credit repair businesses can and cannot do. For example: you cannot collect payment before completing services, you must provide a written contract, and you must give clients a 3-day right to cancel. This section manages your required disclosures, state licensing status, and mandatory client agreement templates. Staying CROA-compliant protects you legally and is a powerful trust signal to clients." }
    ]
  },
  'client-invoicing': {
  ...
  'affiliates-admin': {
    ...
    icon: '🛠️',
    desc: 'Manage all affiliates, set commission rates, and review payouts',
    steps: [
      { target: '#ratesPanel', title: 'Affiliate Admin: Manage Your Referral Program', body: "This is the admin control panel for the Evolv Affiliate Program. From here, you set the commission rates that affiliate members earn when they refer someone who signs up. You also see every affiliate in your program, their referral counts, earnings, and payout history. Think of it as the back-office for your referral marketing engine." },
      { target: '#rate_diy_basic, #rate_diy_pro, #rate_diy_tradeline', title: 'DIY Product Commission Rates', body: "Set how much affiliates earn for referring signups to the DIY consumer product — Evolv's self-service credit repair app for individual consumers. Set the flat dollar amount for a Basic plan signup, the amount for a Pro plan signup, and the percentage for Tradeline purchases. Higher rates attract more active affiliates, but should still leave healthy profit margins for the business." },
      { target: '#rate_crm_starter, #rate_crm_business, #rate_crm_enterprise', title: 'CRM Commission Rates: Your Biggest Earners', body: "These rates apply when an affiliate refers someone who signs up for the professional CRM — the tool you're using right now. Starter plan signups, Business plan signups, and Enterprise plan signups all have separate rates. CRM plans are typically the highest-value referrals ($97–$597/mo subscription) so these commissions tend to be the most motivating for high-performing affiliates." },
      { target: '#btnSaveRates, #ratesSaveStatus', title: 'Save Commission Rates', body: "After setting your commission amounts, click Save Rates to make them live. The green 'Saved!' confirmation appears briefly to confirm changes were applied. Affiliates see the updated rates in their dashboard immediately. You can adjust rates at any time — if you want to run a special promotion (e.g., double commissions for the next 30 days), simply update here and notify affiliates." },
      { target: '#affList, #affCards, #affEmpty', title: 'Affiliate Member List: Your Referral Network', body: "This section shows every person who has joined your affiliate program. Each card displays their name, how many referral link clicks they've generated, how many signups they've converted, their total earnings, and their payout status. Use this to identify your top performers (reward them with bonus rates or exclusive perks) and inactive affiliates (re-engage them with updated marketing materials or personal outreach)." }
    ]
  },
  affiliate: {
  ...
  'marketing-sms': {
    ...
    steps: [
      ...
      { target: '#smsTemplateForm, #smsTemplateList', title: 'SMS Templates: Save, Reuse, Stay Compliant', body: "Save your best messages as named templates organized by audience segment (Leads, Active Clients, Past Clients). Every SMS campaign must include opt-out language — 'Reply STOP to unsubscribe' — which is legally required under TCPA (Telephone Consumer Protection Act — the federal law governing unsolicited text messages). The system reminds you if it's missing before sending. Templates sync directly to your SMS automation backend so they're ready for automated workflows too." }
    ]
  },
  'marketing-email': {
...
const PAGE_MAP = {
  ...
  'affiliates-admin': 'affiliates-admin', 'affiliates-admin.html': 'affiliates-admin',
...
const ROUTE_MAP = {
  ...
  'affiliate': '/affiliate', 'affiliates-admin': '/affiliates-admin',
  'social': '/facebook-manager',
