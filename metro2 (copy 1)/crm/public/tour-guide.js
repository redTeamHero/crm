...
const PAGE_TOURS = {
  ...
  clients: {
    ...
    steps: [
      { target: '.client-search, [data-tour="client-search"], #searchConsumers', title: 'Client Search: Instant Client Lookup', body: "Type any part of a client's name, email, phone number, or status and your list filters instantly. With hundreds of clients, this becomes your most-used tool. Pro tip: you can also search by creditor name to find all clients disputing with a specific bank." },
      { target: '.add-consumer-btn, [data-tour="add-client"], #addConsumerBtn', title: 'Onboard New Clients', body: "Click this button to add a new client to your system. Once added, you can immediately start uploading their credit reports, running audits, and generating dispute letters. The faster you onboard, the faster you can start delivering results." },
      { target: '#consumersList, .consumer-table, [data-tour="client-list"]', title: 'Client Pipeline: Your Complete Client List', body: "Every person you're helping is displayed with their key info: name, status, bureau data, number of disputes, and progress percentage. Click any row to open their full profile. Status badges (Active, Pending, Completed) help you instantly see who needs attention." },
      { target: '.bank-filter, [data-tour="bank-filter"], #bankFilterSelect', title: 'Advanced Filtering', body: "Smart filters let you slice your client list by creditor/bank, dispute status, bureau, or custom tags. Combining filters helps you batch your work efficiently — handle all similar cases together instead of jumping between different types." },
      { target: '.bulk-actions, [data-tour="bulk-actions"], .action-bar', title: 'Bulk Actions: Work at Scale', body: "Select several clients using the checkboxes, then choose an action: generate letters in batch, update statuses, export data, or send notifications. Credit repair pros who master bulk actions can handle 3-5x more clients." }
    ]
  },
  leads: {
    label: 'Leads',
  ...
  disputes: {
    ...
    steps: [
      { target: '.workspace-hero, .hero-title', title: 'Dispute Tracker: Your Command Center', body: "A single view of every dispute round, bureau response, and letter sent for any client. Select a client to load their full dispute history and manage the next steps in their credit repair journey." },
      { target: '#consumerPicker', title: 'Select a Client', body: "Use the dropdown to pick any client in your roster. Once selected, their full dispute tracker loads below — showing all rounds, the timeline of bureau interactions, and every letter that's been sent on their behalf." },
      { target: '#disputeAnalysisCard', title: 'Report Analysis Card: AI-Powered Strategy', body: "This card shows the AI-powered analysis of their credit report. It highlights the strongest dispute arguments, flags which items have the highest deletion potential, and gives you a strategic roadmap for each round." },
      { target: '#letterHistoryCard', title: 'Letters Sent History: Your Audit Trail', body: "This card tracks every dispute letter generated and sent for this client — letter type, bureau targeted, and date sent. If a bureau claims they never received correspondence, this log proves otherwise." },
      { target: '#disputeTimeline', title: 'Dispute Timeline: The Heart of the Tracker', body: "Each entry shows a dispute round with its status — Pending, Awaiting Response, Resolved, or Deleted. Select multiple entries using checkboxes and use the toolbar to generate next-round letters, mark items as resolved, or download a full round." }
    ]
  },
  cfpb: {
    ...
    steps: [
      { target: '.workspace-hero, .hero-title', title: 'CFPB Complaint Generator: Federal-Level Power', body: "When bureau disputes fail — or a creditor refuses to comply — the CFPB is your next weapon. Filing a CFPB complaint puts federal oversight on the case and often produces results that dispute letters alone cannot. This tool drafts a legally-precise complaint using AI in seconds." },
      { target: '#consumerSelect', title: 'Pick the Client', body: "Select the client you're generating the complaint for. The form will use their name and contact details in the complaint narrative automatically. Their negative items also load into the items panel so you can select exactly which accounts are part of the complaint." },
      { target: '#cfpbFormSection', title: 'Complaint Details Form', body: "Fill in the company being complained about (Equifax, Experian, TransUnion, or any creditor), the violation type (FCRA no response, inaccurate reporting, re-aged debt, and more), and attach any proof documents like dispute letters and bureau responses." },
      { target: '#cfpbItemsPanel', title: 'Disputed Account Items: Be Specific', body: "Check the specific accounts or items you want included in the complaint. These are pulled directly from the client's credit report data. Being specific about which accounts are affected makes the complaint much harder for the bureau to dismiss." },
      { target: '#btnGenerateCfpb', title: 'Generate & Save', body: "Click Generate to have AI write a formal 'What Happened' narrative and a 'What Resolution I Am Seeking' statement — both using FCRA-compliant language. Review the output, copy it to consumerfinance.gov, and click Save to Record to attach it to the client's file." }
    ]
  },
  education: {
    ...
    steps: [
      { target: '.workspace-hero, .hero-title', title: 'Credit Academy: Master Your Craft', body: "The Credit Academy is your personal training ground for becoming a world-class credit repair professional. Structured lessons cover everything from credit report basics and FCRA law fundamentals all the way to advanced Metro-2 dispute strategy and business scaling." },
      { target: '.edu-header', title: 'Your Learning Progress', body: "Track your growth here. Your Level badge shows your current rank as you earn XP from completing lessons. The XP progress bar fills as you learn. Your streak counter tracks consecutive days of study — consistency is the fastest path to mastery." },
      { target: '#educationSection, #education', title: 'Course Catalog: Structured Learning', body: "Lessons are organized by skill level — Beginner, Intermediate, and Expert. Beginner covers credit report anatomy and initial dispute strategy. Intermediate dives into FCRA law and letter writing. Expert covers Metro-2 compliance and advanced escalation strategies." },
      { target: '.edu-xp-bar, .edu-xp-fill', title: 'XP & Level System', body: "Every lesson you complete earns XP points. As your XP grows, you level up — from Beginner through Practitioner to Expert and beyond. Higher levels signal to your clients and peers that you've put in the work." }
    ]
  },
  affiliate: {
  ...
  social: {
    ...
    steps: [
      { target: '.workspace-hero, .hero-title', title: 'Social Media Manager: Stay Top-of-Mind', body: "Stay visible with your audience without spending hours creating content. The Social Media Manager connects to your Facebook Page, pulls articles from RSS news feeds, and uses AI to craft compliant, engaging posts — ready to schedule with one click." },
      { target: '#smTabs', title: 'Section Tabs: Navigate Your Tools', body: "Navigate using the tabs: Connect links your Facebook Page, Feeds manages the RSS news sources you pull content from, Compose creates individual posts with AI assistance, and Schedule shows your upcoming post calendar." },
      { target: '#tab-connect, #connectBody', title: 'Facebook Connection', body: "Connect your Facebook Page using your Facebook App credentials. Once connected, the status badge turns green and your Page name appears. All posts generated here go to this connected Page. The setup guide walks you through creating a Facebook App — it takes about 5 minutes." },
      { target: '#tab-feeds, #feedsList', title: 'RSS Feed Sources: Fresh Content Automatically', body: "Add any RSS feed URL and give it a name — CFPB News, NerdWallet, or your favorite credit industry blog. The system pulls the latest articles from each feed so you always have fresh, relevant content to share." }
    ]
  },
  'marketing-sms': {
    label: 'Marketing — SMS',
...
class EvolvTourEngine {
  ...
  _showStep(index) {
    ...
    if (target) {
      ...
      const vPad = 80;
      const inView = rect.top >= vPad && rect.bottom <= window.innerHeight - vPad;
