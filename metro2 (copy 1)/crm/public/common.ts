...
declare global {
  interface Window {
    ...
    registerTourAvailability: () => void;
    setFloatingTourState: (state: unknown) => void;
    ...
    EvolvTour?: { showMenu: () => void; [key: string]: unknown };
  }
}

// ---------------------------------------------------------------------------
// Core types exported for use by other modules
// ---------------------------------------------------------------------------
...
export type ApiResponse<T extends Record<string, unknown> = Record<string, unknown>> =
  T & { status: number; ok: boolean; error?: string };
...
function initResponsiveNav() {
  ...
  if (!nav.querySelector('.nav-close-row')) {
  ...
  const closeDropdowns = (except?: unknown) => {
    dropdowns.forEach((entry) => {
      if (!entry) return;
  ...
  document.addEventListener('click', (event) => {
    dropdowns.forEach((entry) => {
      if (!entry) return;
      if (!entry.dropdown.contains(event.target as Node)) {
        entry.dropdown.classList.remove('open');
        entry.control.setAttribute('aria-expanded', 'false');
      }
    });
...
async function fallbackInviteFlow(sourceId){
  ...
  if (!name) return;
  try {
    const member = await createTeamMember({ name, email }) as TeamMember;
...
function attachInviteHandlers(){
  ...
  document.querySelectorAll<HTMLElement>('[data-action="invite-team"]').forEach(btn => triggers.add(btn));
...
function restrictRoutes(role){
  ...
  if(!ok){
    location.href = role === 'client' ? '/client-portal' : '/dashboard';
  }
}
...
function highlightActiveTheme(name) {
  ...
  if (!palette) return;
  const bubbles = palette.querySelectorAll<HTMLElement>('.bubble');
...
function applyTheme(name){
  ...
  const slider = document.getElementById('glassAlpha') as HTMLInputElement | null;
...
function initPalette(){
  ...
  const toggle = wrap.querySelector<HTMLElement>('.toggle')!;
  const controls = wrap.querySelector('.palette-controls');
  const icon = toggle.querySelector('.toggle-icon');
  const label = toggle.querySelector('.toggle-label');
  const slider = wrap.querySelector<HTMLInputElement>('#glassAlpha');
  ...
  const syncState = () => {
    ...
    if (slider) {
      ...
      (slider as HTMLInputElement).disabled = isCollapsed;
    }
  ...
  wrap.addEventListener('click', (e)=>{
    const b = (e.target as HTMLElement).closest<HTMLElement>('.bubble');
    if(!b) return;
  ...
  if(slider){
    ...
    if (!Number.isNaN(storedAlpha)) {
      slider.value = String(storedAlpha);
    ...
    slider.addEventListener('input', e=>{
      ...
      setGlassAlpha(v);
      localStorage.setItem('glassAlpha', String(v));
    });
  }
}
 ...
 */
export async function api<T extends Record<string, unknown> = Record<string, unknown>>(
  url: string,
  ...
  try {
    ...
    try {
      ...
      return { status: res.status, ok: res.ok, data: text } as unknown as ApiResponse<T>;
    }
    if (res.status === 401 || (res.status === 403 && parsed && parsed.error === 'Forbidden')) {
      ...
      return { status: res.status, ok: false, ...(parsed || {}) } as unknown as ApiResponse<T>;
    }
    if (parsed && typeof parsed === 'object') {
      return { status: res.status, ...parsed } as unknown as ApiResponse<T>;
    }
    return { status: res.status, data: parsed } as unknown as ApiResponse<T>;
  } catch (err) {
    clearTimeout(timer);
    const isTimeout = (err as Error)?.name === 'AbortError';
    return { ok: false, status: 0, error: isTimeout ? 'Request timed out. Check your connection and refresh.' : String(err) } as unknown as ApiResponse<T>;
  }
}

export async function createTeamMember({ name = '', email = '', password = '', teamRole = '' } = {}) {
  const payload: Record<string, unknown> = { username: email, name };
  ...
  if (!res.ok || !res.member) {
    throw new Error((res.error as string) || 'Failed to create team member');
  }
  const m = res.member as Record<string, unknown>;
  const entry = {
    id: m.id,
    name: m.name || name || m.email || email,
    email: m.email || email,
    role: 'team',
    teamRole: m.teamRole || teamRole || 'analyst',
    roleLabel: m.roleLabel || '',
    roleDescription: m.roleDescription || '',
    roleDescriptionEs: m.roleDescriptionEs || '',
...
export async function updateTeamMemberRole(id, teamRole) {
  ...
  if (!res.ok || !res.member) {
    throw new Error((res.error as string) || 'Failed to update team member');
  }
  const rm = res.member as Record<string, unknown>;
  const team = JSON.parse(localStorage.getItem('teamMembers') || '[]').map((member: Record<string, unknown>) => {
    if (member.id !== rm.id) return member;
    return {
      ...member,
      teamRole: rm.teamRole || teamRole,
      roleLabel: rm.roleLabel || member.roleLabel || '',
      roleDescription: rm.roleDescription || member.roleDescription || '',
      roleDescriptionEs: rm.roleDescriptionEs || member.roleDescriptionEs || '',
      permissions: Array.isArray(rm.permissions) ? rm.permissions : member.permissions || [],
    };
  });
  localStorage.setItem('teamMembers', JSON.stringify(team));
  return res.member;
...
function bindHelp(){
  ...
  document.getElementById('helpModal')?.addEventListener('click', (e)=>{ if((e.target as HTMLElement).id==='helpModal') closeHelp(); });
...
window.setHelpGuideState = function(state: Record<string, unknown> = {}){
  ensureHelpModal();
  const tourButton = document.getElementById('helpTourButton');
  const resetButton = document.getElementById('helpTourReset');
  const status = document.getElementById('helpTourStatus');
  const mode = (state.mode as string) || 'start';
  if(tourButton){
  ...
  if(resetButton){
    const showReset = mode === 'resume' || Boolean(state.completed);
...
function initBackToTop(){
  ...
  btn.innerHTML = '<svg viewBox="0 0 100 100" width="28" height="28" xmlns="http://www.w3.org/2000/svg"><g transform="translate(50,50)"><g class="fab-wing-l"><path d="M-5,-5 C-25,-35 -50,-30 -45,-10 C-42,2 -25,8 -5,2 Z" fill="#0a0a0a" opacity="0.9"/><path d="M-5,5 C-25,30 -45,28 -40,12 C-37,2 -22,-2 -5,2 Z" fill="#1a1a1a" opacity="0.85"/></g><g class="fab-wing-r"><path d="M5,-5 C25,-35 50,-30 45,-10 C42,2 25,8 5,2 Z" fill="#0a0a0a" opacity="0.9"/><path d="M5,5 C25,30 45,28 40,12 C37,2 22,-2 5,2 Z" fill="#1a1a1a" opacity="0.85"/></g><ellipse cx="0" cy="0" rx="3.5" ry="12" fill="#0a0a0a"/><circle cx="-1.5" cy="-8" r="1.8" fill="#0a0a0a"/><circle cx="1.5" cy="-8" r="1.8" fill="#0a0a0a"/><line x1="-2" y1="-12" x2="-6" y2="-20" stroke="#0a0a0a" stroke-width="0.8" stroke-linecap="round"/><line x1="2" y1="-12" x2="6" y2="-20" stroke="#0a0a0a" stroke-width="0.8" stroke-linecap="round"/><circle cx="-6" cy="-21" r="1.2" fill="#0a0a0a"/><circle cx="6" cy="-21" r="1.2" fill="#0a0a0a"/></g></svg>';
...
(function initScrollUnlock() {
  ...
  function hasVisibleModal() {
    var els = document.querySelectorAll<HTMLElement>(MODAL_SELECTORS);
