...
/** A single recommendation from /api/consumers/:id/disputes/:jobId/recommendation */
interface DisputeRec {
  creditor: string;
  bureau: string;
  /** Index into the consumer's tradeline array (null when unknown) */
  tradelineIndex: number | null;
  /** Index of the specific item within the dispute round */
  itemIndex: number | null;
  recommendedTemplate: string | null;
  /** 'bureau' → send to credit bureau; 'collector' → send directly to collector */
  letterTarget: 'bureau' | 'collector';
  urgency: 'high' | 'medium' | 'low' | string;
  resolved: boolean;
  reason: string;
  specificDisputeReason: string | null;
  /** Collector/creditor name used when letterTarget === 'collector' */
  collectorName?: string;
}

/** One entry in the deduped collector list sent to the generation endpoint */
...
/** A group of DisputeRecs sharing the same tradeline (used in the target modal) */
interface TargetGroup {
  creditor: string;
  bureaus: string[];
  templates: string[];
  /** Indices into the original recs array for all members of this group */
  indices: number[];
}

/** An entry in selMap — bureau-side tradeline selections */
interface SelMapEntry {
...
/** A letter template from /api/sample-letters */
interface LetterTemplate {
  id: string;
  name: string;
}

/** Shape of data returned by /api/consumers/:id/disputes */
...
let currentDisputeData: DisputeData | null = null;
let disputePollTimer: ReturnType<typeof setInterval> | null = null;
const disputeTemplateOverrides: Record<string, string> = {};
const selectedItems = new Set<string>();
const collapsedRounds = new Set<string>();
...
let DISPUTE_LETTER_TEMPLATES: LetterTemplate[] = [];
...
async function loadConsumers() {
  ...
  try {
    ...
    if (clientId) {
      (picker as HTMLSelectElement).value = clientId;
...
async function loadDisputeTracker() {
  ...
  try {
    ...
    currentDisputeData = data as unknown as DisputeData;
    panel.classList.remove('hidden');
    renderDisputeTracker(data as unknown as DisputeData);
  } catch (err) {
...
function showNextRoundTargetModal(recs: DisputeRec[]): Promise<string[] | null> {
  return new Promise((resolve) => {
    ...
    const groups: TargetGroup[] = [...groupMap.values()];
    ...
    function refreshRow(gidx) {
      ...
      if (!row) return;
      row.querySelectorAll<HTMLElement>('.nrt-toggle').forEach(btn => {
        const isBureauBtn = (btn as HTMLElement & { dataset: DOMStringMap }).dataset.target === 'bureau';
    ...
    modal.addEventListener('click', (e) => {
      const btn = (e.target as HTMLElement).closest<HTMLElement>('.nrt-toggle');
      if (btn) {
        const gidx = parseInt(btn.dataset.gidx, 10);
        const target = btn.dataset.target;
        // Apply chosen target to ALL recs in the group at once.
        groups[gidx].indices.forEach(i => { targets[i] = target as 'bureau' | 'collector'; });
        refreshRow(gidx);
        return;
      }
...
function openLetterPreviewModal(letterJobId, letters, roundNum, portalSent, portalError) {
  ...
  function updateLpmCalc() {
    ...
    const dlBtn = modal.querySelector<HTMLButtonElement>('#lpmDownloadSelected');
    if (dlBtn) {
      dlBtn.textContent = `⬇ Selected (${count})`;
      dlBtn.disabled = count === 0;
      dlBtn.style.opacity = count === 0 ? '0.4' : '1';
    }
    const allCb = modal.querySelector<HTMLInputElement>('#lpmSelectAll');
    if (allCb) {
  ...
  modal.querySelectorAll<HTMLElement>('.lpm-mail-type').forEach(btn => {
    btn.addEventListener('click', () => {
      activeMailKey = btn.dataset.key;
      modal.querySelectorAll<HTMLElement>('.lpm-mail-type').forEach(b => {
  ...
  modal.querySelectorAll<HTMLInputElement>('.lpm-letter-check').forEach(cb => {
    cb.addEventListener('change', (e) => {
      const tgt = e.target as HTMLInputElement;
      const idx = parseInt(tgt.dataset.idx, 10);
      const card = tgt.closest('.lpm-letter-card');
      if (tgt.checked) {
        letterSelections.add(idx);
        if (card) (card as HTMLElement).style.borderColor = 'rgba(212,168,83,0.45)';
      } else {
        letterSelections.delete(idx);
        if (card) (card as HTMLElement).style.borderColor = 'rgba(212,168,83,0.15)';
      }
  ...
  modal.querySelector('#lpmSelectAll')?.addEventListener('change', (e) => {
    ...
    modal.querySelectorAll<HTMLInputElement>('.lpm-letter-check').forEach(cb => {
      ...
      if (card) (card as HTMLElement).style.borderColor = tgt.checked ? 'rgba(212,168,83,0.45)' : 'rgba(212,168,83,0.15)';
    });
    if (groupingActive) { renderGroupView(); updateGroupCalc(); } else { updateLpmCalc(); }
  });
  ...
  modal.querySelector('#lpmDownloadSelected')?.addEventListener('click', async () => {
    ...
    const btn = modal.querySelector<HTMLButtonElement>('#lpmDownloadSelected');
  ...
  const letterGrid    = modal.querySelector<HTMLElement>('#lpmLetterGrid');
  const groupWrap     = modal.querySelector<HTMLElement>('#lpmGroupView');
  const groupToggleBtn = modal.querySelector<HTMLButtonElement>('#lpmGroupToggle');
  const groupedDlBtn  = modal.querySelector<HTMLButtonElement>('#lpmDownloadGrouped');
  ...
  function renderGroupView() {
    ...
    groupWrap.querySelectorAll<HTMLElement>('.lpm-group-header').forEach(hdr => {
      hdr.addEventListener('click', () => {
        const key = hdr.dataset.key;
        const wasCollapsed = _groupCollapsed.get(key) !== false;
        _groupCollapsed.set(key, !wasCollapsed);
        const body = groupWrap.querySelector<HTMLElement>(`.lpm-group-body[data-key="${CSS.escape(key)}"]`);
        const chev = hdr.querySelector<HTMLElement>('.lpm-group-chevron');
        if (body) {
          if (wasCollapsed) {
            body.style.maxHeight = '200px';
            body.style.padding = '0 14px 10px';
            body.querySelector<HTMLElement>('div').style.paddingTop = '4px';
            if (chev) { chev.style.transform = 'rotate(180deg)'; chev.textContent = '▲'; }
          } else {
            body.style.maxHeight = '0';
            body.style.padding = '0 14px 0';
            body.querySelector<HTMLElement>('div').style.paddingTop = '0';
  ...
  modal.querySelectorAll<HTMLElement>('.lpm-view').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const url = btn.dataset.url;
      const overlay = modal.querySelector<HTMLElement>('#lpmIframeOverlay');
      const iframe = modal.querySelector<HTMLIFrameElement>('#lpmIframe');
      iframe.src = url;
      overlay.style.display = 'flex';
    });
  });

  const iframeOverlay = modal.querySelector<HTMLElement>('#lpmIframeOverlay');
  modal.querySelector('#lpmIframeClose').addEventListener('click', () => {
    iframeOverlay.style.display = 'none';
    modal.querySelector<HTMLIFrameElement>('#lpmIframe').src = '';
  });

  modal.querySelector('#lpmSendPortal').addEventListener('click', async () => {
    const btn = modal.querySelector<HTMLButtonElement>('#lpmSendPortal');
    let errEl = modal.querySelector<HTMLElement>('#lpmPortalError');
    btn.disabled = true;
...
let _sentTemplatesByCreditorBureau: Record<string, Set<string>> = {};
...
function renderDisputeTracker(data) {
  ...
  const timeline = document.querySelector<HTMLElement>('#disputeTimeline')!
  ...
  timeline.querySelectorAll<HTMLElement>('.dispute-round-header').forEach(header => {
    header.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).closest('input, select, button, .dispute-sent-date')) return;
      const jid = header.dataset.jobId;
      const isResolved = header.dataset.resolved === 'true';
      const body = timeline.querySelector<HTMLElement>(`.dispute-round-body[data-job-id="${jid}"]`);
  ...
  timeline.querySelectorAll<HTMLInputElement>('.dispute-item-check').forEach(cb => {
    cb.addEventListener('change', (e) => {
      e.stopPropagation();
      const tgt = e.target as HTMLInputElement;
      toggleItemSelection(tgt.dataset.jobId, parseInt(tgt.dataset.itemIndex, 10), tgt);
    });
  });

  timeline.querySelectorAll<HTMLElement>('.dispute-item-row').forEach(row => {
    row.addEventListener('click', (e) => {
      if ((e.target as HTMLElement).closest('input, select, button, a, .dispute-item-status-wrap')) return;
      const details = row.querySelector<HTMLElement>('.dispute-item-details');
      const chevron = row.querySelector<HTMLElement>('.dispute-item-chevron');
      if (details) {
  ...
  timeline.querySelectorAll<HTMLButtonElement>('.btn-cfpb-round').forEach(btn => {
  ...
  timeline.querySelectorAll<HTMLElement>('.dispute-sent-date').forEach(span => {
  ...
  timeline.querySelectorAll<HTMLInputElement>('.dispute-select-all').forEach(cb => {
    cb.addEventListener('change', (e) => {
      const tgt = e.target as HTMLInputElement;
      const jobId = tgt.dataset.jobId;
      const round = currentDisputeData?.rounds?.find(r => r.jobId === jobId);
      ...
      const container = tgt.closest('.glass.card') || tgt.closest('.panel');
      if (container) {
        container.querySelectorAll<HTMLInputElement>('.dispute-item-check').forEach(itemCb => {
          itemCb.checked = tgt.checked;
          const row = itemCb.closest<HTMLElement>('.dispute-item-row');
          if (row) row.style.background = tgt.checked ? 'rgba(212,168,83,0.1)' : 'rgba(255,255,255,0.03)';
        });
      }
      updateSelectionToolbar();
    });
  ...
  timeline.querySelectorAll<HTMLInputElement>('.dispute-followup-days').forEach(input => {
    input.addEventListener('change', async (e) => {
      const tgt = e.target as HTMLInputElement;
      const jobId = tgt.dataset.jobId;
      const days = parseInt(tgt.value, 10);
  ...
  timeline.querySelectorAll<HTMLInputElement>('.dispute-item-followup-days').forEach(input => {
    input.addEventListener('change', async (e) => {
      const tgt = e.target as HTMLInputElement;
      const jobId = tgt.dataset.jobId;
      const itemIndex = parseInt(tgt.dataset.itemIndex, 10);
      const days = parseInt(tgt.value, 10);
  ...
  timeline.querySelectorAll<HTMLSelectElement>('.dispute-template-override').forEach(sel => {
    sel.addEventListener('change', (e) => {
      const tgt = e.target as HTMLSelectElement;
      const key = `${tgt.dataset.jobId}__${tgt.dataset.itemIndex}`;
      if (tgt.value) {
        disputeTemplateOverrides[key] = tgt.value;
        tgt.style.color = '#60a5fa';
      } else {
        delete disputeTemplateOverrides[key];
        tgt.style.color = '#9ca3af';
      }
    });
  });
...
$('#consumerPicker')?.addEventListener('change', (e) => {
  selectConsumer((e.target as HTMLSelectElement).value);
});
...
document.addEventListener('change', async (e) => {
  ...
  if (!sel.classList.contains('dispute-item-status-select')) return;
...
function dpUpdatePreflightGenerateBtn(modal: HTMLElement) {
  const rows = [...modal.querySelectorAll<HTMLElement>('.pf-row')];
  const allResolved = rows.every(row => {
    const ri = row.dataset.row;
    return row.querySelector<HTMLInputElement>(`.pf-addr1[data-row="${ri}"]`)?.value?.trim() &&
           row.querySelector<HTMLInputElement>(`.pf-city[data-row="${ri}"]`)?.value?.trim() &&
           row.querySelector<HTMLInputElement>(`.pf-state[data-row="${ri}"]`)?.value?.trim();
  });
  const genBtn = document.getElementById('addrPreflightGenerate') as HTMLButtonElement | null;
  if (genBtn) {
...
function dpOpenAddrPreflightModal(flagged, enrichedAll, consumerId) {
  return new Promise((resolve) => {
    ...
    const genBtn = document.getElementById('addrPreflightGenerate') as HTMLButtonElement | null;
    const cancelBtn = document.getElementById('addrPreflightCancel') as HTMLButtonElement | null;
    const closeBtn = document.getElementById('addrPreflightClose') as HTMLButtonElement | null;
    const msgEl = document.getElementById('addrPreflightMsg');
    ...
    dpLoadAddrLibrary().then(library => {
      ...
      flagged.forEach((col, i) => {
        ...
        if (col.addr1 && col.addr1 !== '[Add collector address — required before mailing]') {
          const a1 = row.querySelector<HTMLInputElement>(`.pf-addr1[data-row="${i}"]`); if (a1) a1.value = col.addr1;
        }
        if (col.addr2) { const a2 = row.querySelector<HTMLInputElement>(`.pf-addr2[data-row="${i}"]`); if (a2) a2.value = col.addr2; }
        if (col.city) { const c = row.querySelector<HTMLInputElement>(`.pf-city[data-row="${i}"]`); if (c) c.value = col.city; }
        if (col.state) { const s = row.querySelector<HTMLInputElement>(`.pf-state[data-row="${i}"]`); if (s) s.value = col.state; }
        if (col.zip) { const z = row.querySelector<HTMLInputElement>(`.pf-zip[data-row="${i}"]`); if (z) z.value = col.zip; }
      });
      dpUpdatePreflightGenerateBtn(modal);
    });
      ...
      const handleGenerate = async () => {
        ...
        try {
          ...
          for (let i = 0; i < flagged.length; i++) {
            const row = listEl.querySelector(`.pf-row[data-row="${i}"]`);
            if (!row) continue;
            const addr1Val = row.querySelector<HTMLInputElement>(`.pf-addr1[data-row="${i}"]`)?.value?.trim() || '';
            const addr2Val = row.querySelector<HTMLInputElement>(`.pf-addr2[data-row="${i}"]`)?.value?.trim() || '';
            const cityVal  = row.querySelector<HTMLInputElement>(`.pf-city[data-row="${i}"]`)?.value?.trim() || '';
            const stateVal = row.querySelector<HTMLInputElement>(`.pf-state[data-row="${i}"]`)?.value?.trim().toUpperCase() || '';
            const zipVal   = row.querySelector<HTMLInputElement>(`.pf-zip[data-row="${i}"]`)?.value?.trim() || '';
            const saveChecked = row.querySelector<HTMLInputElement>(`.pf-save[data-row="${i}"]`)?.checked;
...
$('#batchGenerateNext')?.addEventListener('click', async () => {
  ...
  const btn = $('#batchGenerateNext') as HTMLButtonElement;
  const origText = btn.textContent;
  btn.disabled = true;
...
$('#batchClear')?.addEventListener('click', () => {
  selectedItems.clear();
  document.querySelectorAll<HTMLInputElement>('.dispute-item-check').forEach(cb => { cb.checked = false; });
  document.querySelectorAll<HTMLInputElement>('.dispute-select-all').forEach(cb => { cb.checked = false; });
  document.querySelectorAll<HTMLElement>('.dispute-item-row').forEach(row => { row.style.background = 'rgba(255,255,255,0.03)'; });
  updateSelectionToolbar();
});
...
function initConsumerCollectorAddrPanel() {
  ...
  const ccaFName = document.getElementById('ccaFName') as HTMLInputElement | null;
  const ccaFAddr1 = document.getElementById('ccaFAddr1') as HTMLInputElement | null;
  const ccaFAddr2 = document.getElementById('ccaFAddr2') as HTMLInputElement | null;
  const ccaFCity = document.getElementById('ccaFCity') as HTMLInputElement | null;
  const ccaFState = document.getElementById('ccaFState') as HTMLInputElement | null;
  const ccaFZip = document.getElementById('ccaFZip') as HTMLInputElement | null;
  const ccaFErr = document.getElementById('ccaFErr');
  const ccaFSave = document.getElementById('ccaFSave') as HTMLButtonElement | null;
  ...
  async function loadCcaList(consumerId) {
    ...
    try {
      ...
      ccaList.querySelectorAll<HTMLElement>('.cca-edit').forEach(btn => {
        btn.addEventListener('click', () => {
      ...
      ccaList.querySelectorAll<HTMLElement>('.cca-del').forEach(btn => {
        btn.addEventListener('click', async () => {
