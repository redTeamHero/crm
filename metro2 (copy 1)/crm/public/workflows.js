...
function renderFilters() {
  ...
  segments.forEach((segment) => {
    if (!segmentSelect.querySelector(`option[value="${CSS.escape(segment)}"]`)) {
...
function badgeTemplate(segment) {
  return `<span class="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-600">${esc(segment)}</span>`;
}

function listTemplate(items) {
  return items.map((item) => `<li class="flex gap-2 text-sm text-slate-600"><span class="text-slate-400">•</span><span>${esc(item)}</span></li>`).join('');
}
      ...
      article.innerHTML = `
        <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
          <div class="flex items-start gap-3">
            <span class="text-3xl" aria-hidden="true">${esc(wf.icon)}</span>
            <div>
              <h2 class="text-xl font-semibold">${esc(wf.title)}</h2>
              <p class="text-sm text-slate-600">${esc(wf.purpose)}</p>
            </div>
          </div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <div class="space-y-3">
            <p class="text-sm font-semibold text-slate-500">${esc(wf.trigger)}</p>
            <ol class="space-y-2">${stepTemplate(wf.steps)}</ol>
            <p class="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700">${esc(wf.outcome)}</p>
          </div>
          <div class="space-y-3">
            <div>
