...
function setField(target, field, value, { overwrite = true } = {}) {
  ...
  if (DATE_FIELDS.has(field)) {
    ...
    const iso = typeof value === 'string' ? value : new Date(value).toISOString().slice(0, 10);
    target[field] = iso;
...
function buildOntologyMaps(graph) {
  ...
  return { ontologyMapById, ontologyMapByField };
}

export function listRuleDebugViolationCodes(graph = loadKnowledgeGraph()) {
  const relationships = Array.isArray(graph.relationships) ? graph.relationships : [];
  const codes = new Set();
  for (const rel of relationships) {
    if (!rel || String(rel.type || '').toLowerCase() !== 'violation_link') continue;
    if (rel.violation) {
      codes.add(rel.violation);
    }
  }
  return Array.from(codes);
}

export function generateRuleDebugTradelines({ startIndex = 0 } = {}) {
  const graph = loadKnowledgeGraph();
  const violationsMeta = loadMetro2Violations();
  const { ontologyMapById, ontologyMapByField } = buildOntologyMaps(graph);
  ...
  return relationships.map((rel, idx) => {
    ...
    return buildRuleTradeline(rel, meta, startIndex + idx, ontologyMapById, ontologyMapByField);
  });
}

export function buildRuleDebugReport({ startIndex = 0, includeNegativeItems = true } = {}) {
