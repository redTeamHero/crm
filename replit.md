...
- An IntelliFeats dispute tracking and follow-up system that auto-activates on credit report upload when negative items/violations are found (creates `dispute_activated` event with per-item first-letter recommendations from `disputeRecommend.js`). Letter generation creates `dispute_round` events with configurable follow-up reminders. Five dispute API endpoints under `/api/consumers/:id/disputes/` handle listing, settings, per-item response recording, evidence upload (with AI scanning via `disputeAI.js` using OpenAI), and next-letter recommendations. The client portal has a Disputes section (`#disputes`) with follow-up questionnaire, evidence upload, and recommendation display. The CRM detail page has a Dispute Tracker panel with report analysis, dispute timeline, configurable follow-up timing, and quick action buttons. State mutations use `updateEventPayload()` for proper persistence. Dark theme CSS covers both CRM (`dispute-status-badge`) and portal (`dispute-badge`) badge class patterns.
