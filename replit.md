...
- A Master Admin Platform Dashboard (`/master`, `master.html`, `master.js`) accessible only to admin/host users. It provides system-wide metrics (not credit-repair focused): total CRM/DIY users, active tenants, revenue overview (MRR split by CRM vs DIY, total collected, profit/loss), and a news feed broadcast system. API endpoints: `GET /api/master/stats`, `GET /api/master/revenue`, `GET/POST/DELETE /api/master/news`. A read-only `GET /api/diy/news` endpoint serves announcements to DIY users without auth. Platform announcements appear on the CRM dashboard (`#platformAnnouncementsSection`) and DIY dashboard (`#diyAnnouncementsSection`). Sidebar shows a "Platform" link under an "ADMIN" group, filtered by `adminOnly` flag and JWT role detection.
